...
func (g *typesGenerator) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	m := map[string]interface{}{
		...
		"IstioServiceEntryStatus": c.Universe.Type(types.Name{Name: "IstioServiceEntryStatus", Package: "istio.io/api/meta/v1alpha1"}),
	}
	for _, kubeType := range kubeTypes {
		template := kubeTypeTemplate
		if kubeType.Type().Name.Name == "ServiceEntry" {
			template = kubeTypeTemplateServiceEntry
		}
...
const kubeTypeTemplateServiceEntry = `
$- range .RawType.SecondClosestCommentLines $
// $ . $
$- end $
$- range .KubeType.Tags $
...
type $.KubeType.Type|public$ struct {
	...
	Status $.IstioServiceEntryStatus|raw$ ` + "`" + `json:"status"` + "`" + `
}
