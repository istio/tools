...
`protoc-gen-deepcopy` is a plugin for protoc which generates `DeepCopyInto()`
functions for `.pb.go` types annotated with `+kubetype-gen`.  The
implementations simply use `proto.Clone()` to copy the contents of the `in`
