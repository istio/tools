...
func TestConcurrentCommand_UnmarshalJSON(t *testing.T) {
	tests := []struct {
		...
		{
			[]byte(`[{"sleep": {"SleepCommand": [{"Load": {"Min": 0, "Max": 100}, 
				"type": "static", "data": {"time": "1s"}}]}}]`),
			ConcurrentCommand{
				SleepCommand{[]SleepCommandData{{Range{uint64(0), uint64(100)},
					Static, SleepCommandStatic{1 * time.Second}}}},
			},
			nil,
		},
		{
			[]byte(`[{"call": "A"}, 
				{"sleep": {"SleepCommand": [{"Load": {"Min": 0, "Max": 100}, 
		...
		},
		{
			...
			ConcurrentCommand{
				RequestCommand{ServiceName: "A"},
				SleepCommand{[]SleepCommandData{{Range{uint64(0), uint64(100)},
					Distribution, SleepCommandDistribution{"normal",
						distuv.Normal{Mu: 1.0, Sigma: 0.25}}}}},
			},
			nil,
		},
	}
